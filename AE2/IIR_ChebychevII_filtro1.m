...
hold on;
%NEW MASK
plot([fp1 fs1 1.21e3 fs2 fp2],[-Ap -As 0 -As -Ap],'r+');
plot([100 fs1 fs1 fs2 fs2 fs2 fa/2],[-As -As -Ap+1 -Ap+1 -As -As -As],'--m'); % máscara da banda de passagem
plot([fs1/pi fp1 fp1 fp2 fp2 fs2+500],[-1.5*As -1.5*As -Ap -Ap -1.5*As -1.5*As],'--m'); % máscara da banda de rejeição
title('Magnitude de H(z) calculada');
xlabel('Frequências projetadas (kHz)');
ylabel('Magnitude (dB)');
ylim ([-65 5]);xlim([500 1500]);
hold off;
%%
figure(8);
zplane(nz,dz);
