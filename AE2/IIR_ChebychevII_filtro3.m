...
fp1 = 627; %fr. de passagem inferior
fp2 = 711; %fr. de passagem superior
fs1 = 683; %fr. de rejeicao inferior
fs2 = 767; %fr. de rejeicao superior
...
hold on;
%NEW MASK
plot([lp1 lp1-400 l0 ls2 lp2],[-Ap -As 0 -As -Ap],'r+');
plot([0 lp1 lp1 lp2 lp2 10e3],[-2*As -2*As -Ap -Ap -2*As -2*As],'--m'); % máscara da banda de passagem
plot([100 lp2-1100 lp2-1100 ls2 ls2 10e3],[-As -As -Ap+1 -Ap+1 -As -As],'--m'); % máscara da banda de rejeição
title('Magnitude do Filtro Analógico H(s)');
xlabel('Frequências do Filtro em Lamda (Hz)');
ylabel('Magnitude (dB)');
ylim ([-85 5]); xlim([1e3 8e3]);
hold off;
%%
...
hold on;
%NEW MASK
plot([fp1 fp1-48 fs1-20 fs2 fp2],[-Ap -As 0 -As -Ap],'r+');
plot([0 fp1 fp1 fp2 fp2 2e3],[-2*As -2*As -Ap -Ap -2*As -2*As],'--m'); % máscara da banda de passagem
plot([100 fp1-48 fp1-48 fs2 fs2 2e3],[-As -As -Ap+1 -Ap+1 -As -As],'--m'); % máscara da banda de rejeição
title('Magnitude de H(z) calculada');
xlabel('Frequências projetadas (kHz)');
ylabel('Magnitude (dB)');
ylim ([-85 5]);xlim([500 1000]);
hold off;
%%
figure(8);
zplane(nz,dz);
